...
const VerDenunciasFundaciones = ({ denuncias, onActualizarDenuncias }) => {
  ...
  // ==== NUEVO ====
  // Cambiar todas las denuncias de una fundación a un estado
  const handleMarcarTodas = async (idFundacion, nuevoEstado) => {
    const denunciasDeFundacion = denunciasData.filter(d => d.idFundacion === parseInt(idFundacion));
    setUpdating(prev => {
      const actualizaciones = {};
      denunciasDeFundacion.forEach(d => { actualizaciones[d.id] = true; });
      return { ...prev, ...actualizaciones };
    });

    try {
      await Promise.all(
        denunciasDeFundacion.map(d => cambiarEstadoDenunciaFundacion(d.id, nuevoEstado))
      );

      setDenunciasData(prev =>
        prev.map(d =>
          d.idFundacion === parseInt(idFundacion)
      ...
      setUpdating(prev => {
        const actualizaciones = {};
        denunciasDeFundacion.forEach(d => { actualizaciones[d.id] = false; });
        return { ...prev, ...actualizaciones };
      });
    }
  };
