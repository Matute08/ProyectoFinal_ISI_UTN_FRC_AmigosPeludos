...
const Solicitudes = () => {
    ...
    useEffect(() => {
        const fetchFormDataSolicitado = async () => {
            try {
                ...
                // Filtrar las veterinarias en estado "en revisión"
                // const veterinariasEnRevision = publicData.filter(
                //   (veterinaria) => veterinaria.estado === 'en revision'
                // );

                // Guardar las veterinarias en estado "en revisión"
                //setFormularioSolicitado(veterinariasEnRevision);
            } catch (error) {
    ...
    const handleViewPDF = (itemId) => {
        const newTab = window.open(
            `/ver-formulario-solicitud/${itemId}`,
            "_blank"
        );
        newTab.focus();
    };
