...
const SolicitudesVeterinaria = () => {
    ...
    const handleUpdateState = async (formId) => {
        const { value: estadoId } = await Swal.fire({
            title: `¿Vas a modificar el estado de la veterinaria: ${formId}? `,
        ...
        if (estadoId) {
            try {
                ...
                Swal.fire({
                    title: `Estado actualizado correctamente para la veterinaria ${formId}`,
                    icon: "success",
    ...
    const handleViewPDF = (itemId) => {
        const newTab = window.open(
            `/ver-formulario-solicitud-veterinaria/${itemId}`,
            "_blank"
        );
        newTab.focus();
    };
