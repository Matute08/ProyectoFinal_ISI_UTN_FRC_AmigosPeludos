...
const VacunacionMascota = () => {
    ...
    const handleDosisEditada = async () => {
        ...
        if (modalHistorial && vacunaSeleccionada) {
            ...
            const dosisActualizadas = vacunasActualizadas.filter(v => {
        ...
        try {
            ...
            switch (tipo) {
                case 'dosis':
                    ...
                    break;
                case 'vacuna':
                    response = await deleteVacunaMascota(mascotaId, datos.vacunaId);
    ...
    return (
        <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            ...
            >
                ...
                >
                    <VaccinesIcon sx={{ fontSize: { xs: "1.8rem", sm: "2.2rem", md: "2.5rem" }, color: "#F4A261" }} />
                ...
                >
                    {mascota ? `Gestión de vacunas para ${mascota.nombre}` : "Gestión de vacunas"}
                </Typography>
            ...
            {/* Vacunas Section */}
                ...
                >
                    ...
                    <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
                        Comenzá cargando la primera dosis para mantener el control de la salud de tu mascota
                        </Typography>
                    <Button
                        variant="contained"
                        size="large"
                ...
                <Box>
                    ...
                    <Grid container spacing={3}>
                        {Object.entries(vacunasAgrupadas).map(([nombre, lista]) => {
                            const ultimaVacuna = lista[0];
                            const aplicada = new Date(ultimaVacuna.fechaAplicacion).toLocaleDateString();
                            const proxima = ultimaVacuna.fechaProxima
                                ? new Date(ultimaVacuna.fechaProxima).toLocaleDateString()
                                : ultimaVacuna.vacuna?.frecuenciaSemanas
                                    ? calcularProxima(ultimaVacuna.fechaAplicacion, ultimaVacuna.vacuna.frecuenciaSemanas)
                                    : "No registrada";

                            const estado = ultimaVacuna.fechaProxima
                                ? new Date(ultimaVacuna.fechaProxima) > new Date()
                                    ? "Al día"
                                    : "Vencida"
                                : ultimaVacuna.vacuna?.frecuenciaSemanas
                                    ? "Al día (estimado)"
                            ...
                            const esVencida = estado.includes("Vencida") || estado.includes("Sin próxima");

                                return (
                                <Grid size={{ xs: 12, md: 6, lg: 4 }} key={nombre}>
                                        <Card
                                        elevation={3}
                                            sx={{
                                                height: "100%",
                                        ...
                                        <CardContent sx={{ p: 3, display: "flex", flexDirection: "column", height: "100%" }}>
                                                        ...
                                                        <IconButton 
                                                            ...
                                                            onClick={async () => {
                                                                ...
                                                                try {
                                                                    ...
                                                                    const vacunaEncontrada = resVacunas.data.find(v => v.nombre === nombre);
                                                                    ...
                                                                    console.log('Vacuna encontrada:', vacunaEncontrada);
                                                                    
                                                                    confirmarEliminacion('vacuna', {
                                                                        vacunaId: vacunaEncontrada.id,
                                                                        nombre: nombre
                                                                    });
                                                                } catch (error) {
                                                                    console.error('Error buscando vacuna:', error);
                                                                    Swal.fire({
                                                                        title: "Error",
                                            ...
                                            <Box sx={{ flexGrow: 1, mb: 2 }}>
                                                <Typography variant="subtitle2" fontWeight="bold" gutterBottom>
                                                    Última dosis (Dosis {lista.length})
                                                            </Typography>
                                                ...
                                                <Stack spacing={1}>
                                                    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                                                        <CalendarTodayIcon sx={{ fontSize: "1rem", color: "text.secondary" }} />
                                                            ...
                                                            <strong>Aplicada:</strong> {aplicada}
                                                            </Typography>
                                                    </Box>
                                            ...
                                            <Box sx={{ mt: "auto" }}>
                                                {lista.length > 1 ? (
                                                    <Button
                                                        ...
                                                        variant="outlined"
                                                        size="small"
                                                        onClick={() => abrirModalHistorial(nombre, lista)}
                                                        startIcon={<Assignment />}
                                                        sx={{ 
                                                            borderColor: "#F4A261",
                                                            color: "#F4A261",
                                                    ...
                                                    <Box sx={{ height: "40px" }} /> // Espacio para mantener altura consistente
                                                )}
                                                                </Box>
                                            </CardContent>
                                        </Card>
                ...
                <DialogTitle sx={{ 
                    ...
                    <Box>
                        ...
                        <Typography variant="caption" sx={{ opacity: 0.9 }}>
                            {vacunaSeleccionada?.lista?.length} dosis registradas
                        </Typography>
                    </Box>
                </DialogTitle>

                <DialogContent sx={{ p: 2, maxHeight: "60vh", overflow: "auto" }}>
                    ...
                    {vacunaSeleccionada && !actualizandoHistorial && (
                        <Stack spacing={1.5} sx={{ mt: 2 }}>
                            {vacunaSeleccionada.lista.map((v, i) => {
                                const dosisNum = vacunaSeleccionada.lista.length - i;
                                const aplicada = new Date(v.fechaAplicacion).toLocaleDateString();
                                const proxima = v.fechaProxima
                                    ? new Date(v.fechaProxima).toLocaleDateString()
                                    : v.vacuna?.frecuenciaSemanas
                                        ? calcularProxima(v.fechaAplicacion, v.vacuna.frecuenciaSemanas)
                                        : "No registrada";
                                ...
                                return (
                                    <Paper 
                                        ...
                                        sx={{ 
                                            ...
                                            borderRadius: 2,
                                            backgroundColor: esUltima ? "#f8f9fa" : "white"
                                        }}
                                    >
                                            ...
                                            <Box sx={{ display: "flex", gap: 0.5 }}>
                                                <Tooltip title="Editar dosis">
                                                    <IconButton 
                                                        ...
                                                        onClick={() => abrirModalEditar({
                                                            ...
                                                            nombreVacuna: vacunaSeleccionada.nombre
                                                        })}
                                                    >
                                                ...
                                                <Tooltip title="Eliminar esta dosis">
                                                    <IconButton 
                                                        size="small" 
                                                        color="error"
                                                        onClick={() => confirmarEliminacion('dosis', { id: v.id })}
                                                    >
                                        ...
                                        <Grid container spacing={1}>
                                            ...
                                            <Grid size={{ xs: 12, sm: 4 }}>
                                                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                                                    {esVencida ? (
                                                        <WarningIcon sx={{ fontSize: "0.9rem", color: "#f44336" }} />
                                                    ) : (
                                                        <CheckCircleIcon sx={{ fontSize: "0.9rem", color: "#4caf50" }} />
                                                    )}
                                                    <Typography variant="caption" color="text.secondary">
                                                        Estado:
                                                    </Typography>
